...
def big() -> None:
    ...
    for simulation_name in folders:
        ...
        time_array = np.concatenate(
            [base_time_scale + idx for idx in range(0, len(files_read))]
        )
...
def small() -> None:
    ...
    for simulation_name in folders:
        ...
        plt.plot(time_array, keffs)
        plt.legend(folders, loc='lower center',  bbox_to_anchor=(0.5, -0.3),
          fancybox=True, shadow=True, ncol=3, fontsize=8)

    plt.xlabel("Shuffling step",fontsize=12)
    plt.ylabel("Keff",fontsize=12)
    plt.grid()
    plt.savefig("keff_vs_days_zoom.png",  dpi=1200, bbox_inches = "tight",)
    plt.show()
    plt.clf() 
