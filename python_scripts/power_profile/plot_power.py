...
boec = []
eoec = []
length = sum([1 for _ in open('./BBR.CORERef0/BBR.CORERef_core0.m')])


#BOEC power density
while a < 73: #number of steps
    with open('./BBR.CORERef'+str(a)+'/BBR.CORERef_core'+str(0)+'.m') as fb:
...
#EOEC power density
    with open('./BBR.CORERef'+str(a)+'/BBR.CORERef_core'+str(4)+'.m') as fe:
...
#plot,save
    ...
    ax.plot(distvec, boec, color = 'r', marker = 'o', markersize=4, linestyle = 'None', label = 'BOEC')
    ax.plot(distvec, eoec, color = 'b', marker = 'o', markersize=4, linestyle = 'None', label = 'EOEC')
